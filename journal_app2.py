...
def view_explanations():
    page_size = 100  # Number of explanations to show per page
    total_explanations = len(bible_explanations)
    current_index = 0

    while True:
        # Calculate the start and end index for the current page
        start_index = max(0, current_index)
        end_index = min(current_index + page_size, total_explanations)
        
        # Display the current page of explanations
        for idx in range(start_index, end_index):
            explanation = bible_explanations[idx]
            print(f"{idx + 1}. Date: {explanation.get('date', 'N/A')}")
        ...
        # Display options for navigating pages
        print(f"Showing explanations {start_index + 1} to {end_index} of {total_explanations}.")
        if start_index > 0:
            print("P: Previous page")
        if end_index < total_explanations:
            print("N: Next page")
        print("Q: Quit viewing")

        # Get the user's choice for navigation
        choice = input("Enter your choice (N/P/Q): ").strip().upper()

        if choice == "N" and end_index < total_explanations:
            current_index += page_size  # Move to the next page
        elif choice == "P" and start_index > 0:
            current_index -= page_size  # Move to the previous page
        elif choice == "Q":
            break
        else:
            print("Invalid choice. Please enter 'N' for next page, 'P' for previous page, or 'Q' to quit.")
